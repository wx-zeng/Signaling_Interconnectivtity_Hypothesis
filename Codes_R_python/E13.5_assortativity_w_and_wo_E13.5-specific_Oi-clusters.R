...
# This master switch controls whether clusters 27/29/30/31 (the E13.5-specific Oi-clusters) are included in the analysis or not.

# omit_clusters <- FALSE  ->  full network (all clusters included)
# omit_clusters <- TRUE   ->  network with E13.5-specific Oi-clusters 27/29/30/31 removed

omit_clusters <- FALSE

clusters_omit <- c("27", "29", "30", "31")

# file-name suffix used for the saved figures when clusters are omitted.
omit_suffix <- if (omit_clusters) "_omit27293031" else ""
...
colors_E9.5 <- colors_all[!names(colors_all) %in% clusters_omit]

# E13.5 keeps all clusters by default; drops 27/29/30/31 when omit_clusters = TRUE.
colors_E13.5 <- if (omit_clusters) {
  colors_all[!names(colors_all) %in% clusters_omit]
} else {
  colors_all
}
...
stage_used  <- "E13.5"   # "E9.5" or "E13.5"
seurat_res <- "0.7"
trim_para  <- "0.05"
...
# Omit clusters 27, 29, 30, 31 from spatial metadata (only when requested)
if (omit_clusters) {
  spatial$meta <- spatial$meta %>%
    filter(!seurat_clusters %in% clusters_omit)
}
...
# Omit clusters 27, 29, 30, 31 from CellChat interactions (only when requested)
if (omit_clusters) {
  df_netP <- df_netP %>%
    filter(!source %in% clusters_omit, !target %in% clusters_omit)
}
...
relative_inter_strength_E13.5 <- relative_inter_strength_all[, "E13.5_ratio"]
names(relative_inter_strength_E13.5) <- rownames(relative_inter_strength_all)

# check
print(relative_inter_strength_E13.5)
...
#  Plots  #
###########
# File names automatically get the "_omit27293031" suffix (via omit_suffix)
# when omit_clusters = TRUE.
...
png(
  file.path(plot_dir_strength, glue("{stage_used}_res={seurat_res}_trim={trim_para}_network_strength.as.weight_node.size=fix_normal_node{omit_suffix}.png")),
...
png(
  file.path(plot_dir_strength, glue("{stage_used}_res={seurat_res}_trim={trim_para}_network_strength.as.weight_node.size=fix_node.color=heat{omit_suffix}.png")),
  width = 4500, height = 5000,
  res = 300,
  bg = "transparent"
)
...
cat("Assortativity coefficient of", stage_used,
    if (omit_clusters) "(omit 27/29/30/31)" else "(full network)",
    "is", assort_coefficient_strength, "\n")